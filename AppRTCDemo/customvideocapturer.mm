...
@interface FrameInputAVFoundationVideoCapturerInternal : NSObject
...
// it.
- (instancetype)initWithCapturer:(webrtc::CustomVideoCapturer *)capturer;
...
@implementation FrameInputAVFoundationVideoCapturerInternal {
  ...
  webrtc::CustomVideoCapturer *_capturer;
...
enum CustomVideoCapturerMessageType : uint32_t {
  kMessageTypeFrame,
};

struct CustomAVFoundationFrame {
  CustomAVFoundationFrame(CVImageBufferRef buffer, int64_t time)
...
CustomVideoCapturer::CustomVideoCapturer()
  ...
  _capturer =
      [[FrameInputAVFoundationVideoCapturerInternal alloc] initWithCapturer:this];
}
...
cricket::CaptureState CustomVideoCapturer::Start(
  ...
  const cricket::VideoFormat* tmpformat = GetCaptureFormat();
...
AVCaptureSession* CustomVideoCapturer::GetCaptureSession() {
  return _capturer.captureSession;
}

bool CustomVideoCapturer::CanUseBackCamera() const {
  return _capturer.canUseBackCamera;
}

void CustomVideoCapturer::SetUseBackCamera(bool useBackCamera) {
  _capturer.useBackCamera = useBackCamera;
}

bool CustomVideoCapturer::GetUseBackCamera() const {
  return _capturer.useBackCamera;
}

void CustomVideoCapturer::CaptureSampleBuffer(
  ...
  CustomAVFoundationFrame frame(image_buffer, rtc::TimeNanos());
  _startThread->Post(this, kMessageTypeFrame,
                     new rtc::TypedMessageData<CustomAVFoundationFrame>(frame));
}

void CustomVideoCapturer::OnMessage(rtc::Message *msg) {
  switch (msg->message_id) {
    case kMessageTypeFrame: {
      rtc::TypedMessageData<CustomAVFoundationFrame>* data =
        static_cast<rtc::TypedMessageData<CustomAVFoundationFrame>*>(msg->pdata);
      const CustomAVFoundationFrame& frame = data->data();
...
void CustomVideoCapturer::OnFrameMessage(CVImageBufferRef image_buffer,
                                               int64_t capture_time) {
