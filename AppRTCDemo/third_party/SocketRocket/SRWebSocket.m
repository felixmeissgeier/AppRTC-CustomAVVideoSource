...
- (NSString *)stringBySHA1ThenBase64Encoding;
{
    return newSHA1String((char *)self.bytes, self.length);
}
...
- (void)_readFrameContinue;
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(SRWebSocket *self, NSData *data) {
        ...
        const uint8_t *headerBuffer = (uint8_t *)data.bytes;
...
- (void)_pumpWriting;
{
    ...
    if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
        NSInteger bytesWritten = [_outputStream write:(uint8_t *)_outputBuffer.bytes + _outputBufferOffset maxLength:dataLength - _outputBufferOffset];
        if (bytesWritten == -1) {
...
- (void)_readUntilBytes:(const void *)bytes length:(size_t)length callback:(data_callback)dataHandler;
{
    ...
    stream_scanner consumer = ^size_t(NSData *data) {
        ...
        const unsigned char *buffer = (unsigned char *)data.bytes;
